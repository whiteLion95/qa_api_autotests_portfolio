...
class DataUtils {
  ...
  static async mapRequestToOnes(getPolicyData, requestData, optionalSchema) {
    ...
    if (JSONMapper.getNestedProperty(flattenedRequestData, 'payment_type').values.pop() === 0) {
      const paymentStartDateFullKey = 'policyData.payments.payment_start_date';
      const nextMonthFirstDay = moment().add(1, 'M').startOf('month').format(JSONLoader.testData.datesFormatDMY);
    ...
    datesFullKeys.push(...JSONMapper.getNestedProperty(mappedData, 'date_end').keys);
    datesFullKeys.push(...JSONMapper.getNestedProperty(mappedData, 'date').keys);
    datesFullKeys.forEach((fullKey) => {
      mappedData[fullKey] = TimeUtils.reformatDateFromYMDToDMY(mappedData[fullKey]);
    });

    const percentFullKeys = JSONMapper.getNestedProperty(mappedData, 'franchise_percent').keys;
    percentFullKeys.push(...JSONMapper.getNestedProperty(mappedData, 'franchise_damage_percent').keys);
    percentFullKeys.push(...JSONMapper.getNestedProperty(mappedData, 'franchise_loss_percent').keys);
    percentFullKeys.forEach((fullKey) => {
      mappedData[fullKey] = Number(mappedData[fullKey].substr(0, mappedData[fullKey].indexOf('%')));
    });
  ...
  static mapESBDToOnes(getPolicyData, getContractByNumberData) {
    this.saveToJSON({ getPolicyData });
    const paymentType = JSONMapper.getNestedProperty(JSONMapper.flattenJSON(getPolicyData), 'payment_form').values.pop();
    let mappedData = JSONMapper.mapValues(
      { getPolicyData },
    ...
    const paymentFullKeys = JSONMapper.getNestedProperty(mappedData, 'payment').keys;
  ...
  static createRandomBeneficiaryAndInsuredStructures(clientsArr) {
    const randomBeneficiaryIndex = Randomizer.getRandomInteger(clientsArr.length - 1);
  ...
  static mapTariffToSetPolicy(tariff) {
    ...
    const trimmedTariffKeys = Object.keys(flattenedTariff)
      .map((key) => key.split('.').pop());
    const trimmedSetPolicyTemplateKeys = Object.keys(flattenedSetPolicyTemplate)
      .map((key) => key.split('.').pop());
    const matchingKeys = new Set(trimmedTariffKeys
      .filter((key) => new Set(trimmedSetPolicyTemplateKeys).has(key)));
    const mappingSchema = Array.from(matchingKeys)
      .reduce((obj, element) => ({ ...obj, [element]: element }), {});
    ...
    const IDFromOptions = JSONMapper.getNestedProperty(flattenedUpdatedSetPolicyTemplate, 'id').values[0];
    const tarifIDOnesFullKey = JSONMapper.getNestedProperty(flattenedUpdatedSetPolicyTemplate, 'tarif_id_1c').keys.pop();
    flattenedUpdatedSetPolicyTemplate[tarifIDOnesFullKey] = IDFromOptions;
    const updatedSetPolicyTemplate = JSONMapper.unflattenJSON(flattenedUpdatedSetPolicyTemplate);
