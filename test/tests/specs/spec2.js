...
const { Logger, Randomizer } = require("@amanat-qa/utils-backend");
...
describe("KASKO API test suite:", async () => {
  ...
  it("Test get-car-average-price:", async () => {
    ...
    response.data.should.be
      .jsonSchema(JSONLoader.getCarAveragePriceResponseSchema)
      .and.containSubset(JSONLoader.templateResponse.getCarAveragePrice);
    response.data.data.title
      .toLowerCase()
      .should.be.equal(title.toLocaleLowerCase());
  });
  ...
  it("Test issue/tariffs:", async () => {
    ...
    response.status.should.be.equal(200);
    response.data.should.be
      .jsonSchema(JSONLoader.issueTariffsResponseSchema)
  ...
  it("Test get-car:", async () => {
    ...
    if (global.withESBD) {
      ...
      response.data.should.be.jsonSchema(
        JSONLoader.getCarWithoutESBDResponseSchema
      );
    }
    response.data.data.reg_num.should.be.equal(
      JSONLoader.templateSetPolicy.cars[0].number
    );
    response.data.data.reg_cert_num.should.be.equal(
      JSONLoader.templateSetPolicy.cars[0].passport_number
    );
  });
  ...
  it("Test upload-file:", async () => {
    ...
    response.status.should.be.equal(200);
    const validPolicies = response.data.data.filter(
      (policy) =>
        policy.ones_status === JSONLoader.dictOnes.policy_status.issued
    );
    const randomPolicy =
      validPolicies[Randomizer.getRandomInteger(validPolicies.length - 1)];
    const fileName = Randomizer.getRandomString(
      true,
      false,
      false,
      false,
    ...
    response = await KASKOAPI.uploadFile(randomPolicy.id, fileName);
    response.status.should.be.equal(200);
    response.data.should.be
      .jsonSchema(JSONLoader.uploadFileResponseSchema)
      .and.containSubset(JSONLoader.templateResponse.uploadFile);
    response.data.data.name.should.include(
      `${randomPolicy.policy_number}_${fileName}`
    );

    const responseGetFile = await docsAPI.getFile(
      randomPolicy.policy_number,
      fileName
    );
    responseGetFile.status.should.be.equal(200);
    responseGetFile.data.should.be
      .jsonSchema(JSONLoader.getFileResponseSchema)
