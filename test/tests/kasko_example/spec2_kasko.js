...
describe('KASKO API test suite:', async () => {
  ...
  it('Test upload-file:', async () => {
    ...
    const validPolicies = response.data.data.filter(
      (policy) => policy.ones_status === JSONLoader.dictOnes.policy_status.issued,
    );
  ...
  afterEach(function () {
    // eslint-disable-line func-names
    if (!JSONLoader.configData.parallel && this.currentTest.state) Logger.log(this.currentTest.state);
  });
});
